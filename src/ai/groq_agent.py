...
class GroqAgent(AIAgent):
    """Groq API を使った投資判断エージェント (無料枠で運用可能)

    推奨モデル: llama-3.3-70b-versatile
    無料枠: 30 req/分、6,000 req/日、131,072 tokens/分
    ...
    def __init__(self):
        ...
        if not settings.GROQ_API_KEY:
            raise ValueError("GROQ_API_KEY が設定されていません。.env に追加してください。")
        self.client = Groq(api_key=settings.GROQ_API_KEY)
        self.model = settings.GROQ_MODEL
        self.input_cost, self.output_cost = settings.groq_token_costs

    def _call_api(self, system: str, user: str, max_tokens: int = 4096) -> tuple[str, int, int, float]:
        """Groq API 呼び出し。(response_text, input_tokens, output_tokens, cost_usd) を返す"""
        response = self.client.chat.completions.create(
            ...
            temperature=0.3,  # 投資判断は低めのtemperatureで安定させる
        )
        ...
        logger.info(
            f"Groq API: model={self.model} in={input_tokens} out={output_tokens} cost=${cost_usd:.4f}"
        )
    ...
    def _extract_json(self, text: str) -> dict:
        ...
        logger.error(f"Failed to parse JSON from Groq response: {text[:200]}")
        return {}
    ...
    def run_morning_analysis(self, context: dict) -> MorningAnalysisResult:
        ...
        logger.info(f"Running morning analysis with Groq ({self.model})...")
    ...
    def run_evening_analysis(self, context: dict) -> EveningDecisionResult:
        ...
        logger.info(f"Running evening analysis with Groq ({self.model})...")
        raw_response, input_tokens, output_tokens, cost = self._call_api(
            system_prompt, user_prompt, max_tokens=4096
        )
        parsed = self._extract_json(raw_response)
    ...
    def run_weekly_review(self, context: dict) -> WeeklyReviewResult:
        ...
        logger.info(f"Running weekly review with Groq ({self.model})...")
        raw_response, input_tokens, output_tokens, cost = self._call_api(
            system_prompt, user_prompt, max_tokens=4096
        )
        parsed = self._extract_json(raw_response)
